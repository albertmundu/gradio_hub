...
if __name__ == "__main__":
    ...
    with gr.Blocks() as demo:
        with gr.Row():
            ...
            radio_btn = gr.Radio(choices=["Sobel","Scharr","Prewitt", "Serpia"], label="Filter Type", value="Serpia")
        
        with gr.Row():
            output_image = gr.Image(type="numpy", label="Output")
            hist_input = gr.Plot(label="Input Histogram")
            hist_output = gr.Plot(label="Output Histogram")

        apply_btn.click(fn=process_image, inputs=[input_image, radio_btn], outputs=[output_image, hist_input, hist_output])
